...
param (
  ...
  [string]$RepositoryPath = "$env:USERPROFILE\Documents\STALKER\MINI-REPACK-MODS\COP-MODDING-FOLDER\COP-VANILLA-MINI-REPACK",
  # Set the location where the mod is built and stored as a folder together with a compressed 7z file.
  [string]$BuildPath = "$env:USERPROFILE\Documents\STALKER\MINI-REPACK-MODS\COP-MODDING-FOLDER\COP_MINI_REPACK_RELEASE"
)
...
function Build-Mod {
  ...
  if (-not ($missing_folder)) {
    ...
    $build_folder = "Mini-Repack-Mod_STALKER-COP-Edition_$build_version"
    ...
    if (($status -eq $true) -and -not ($missing_git)) {
      ...
      $path_list = @(
        @("$build_folder_path\gamedata\configs\weapons\upgrades\optional"),
        @("$build_folder_path\gamedata\configs\misc\outfit_upgrades\optional"),
        @("$build_folder_path\gamedata\configs\environment\weathers\optional")
      )
...
while ($true) {
  Write-Host "Mini-Repack mod STALKER COP Edition." -ForegroundColor Blue
